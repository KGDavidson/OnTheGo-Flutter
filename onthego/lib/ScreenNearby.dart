...
Future<List> fetchCurrentNearbyStops(Position currentLocation) async {
  ...
  if (response.statusCode == 200) {
    return jsonDecode(response.body)['stopPoints']
...
Future<List> fetchArrivalTimes() async {
  ...
  if (response.statusCode == 200) {
    return jsonDecode(response.body)
        .map((arrivalTime) => ArrivalTime.fromJson(arrivalTime))
        .toList();
  } else {
    throw Exception('Failed to load');
  }
...
class _ScreenNearby extends State<ScreenNearby> {
  ...
  void reloadPage() async {
    ...
    await Future.delayed(
        ...
        () => {
              mapController.move(
                  LatLng(mapController.center.latitude + 0.0000000001,
                      mapController.center.longitude + 0.0000000001),
                  mapController.zoom)
            });
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Stack(
        children: <Widget>[
          ListViewPage(
              getCurrentLocationAndFindClosest, getCurrentLocation, reloadPage),
          IgnorePointer(
            child: BackdropFilter(
              filter: ImageFilter.blur(
                  sigmaX: currentBlurValue, sigmaY: currentBlurValue),
              child: Container(
                color: Colors.black.withOpacity(
                    currentBlurValue > 0 ? currentBlurValue / 4 : 0),
              ),
            ),
          ),
          MapView(reloadPage),
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.only(
                      topLeft: Radius.circular(
                          MediaQuery.of(context).size.height * toggleHeight),
      ...
      children: <Widget>[
        AnimatedContainer(
          margin: EdgeInsets.only(
              top: MediaQuery.of(context).size.height * toggleBarHeight),
          duration: Duration(milliseconds: animationDuration),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              borderRadius: BorderRadius.only(
                  bottomRight: Radius.circular(
                      MediaQuery.of(context).size.height * pullTabHeight),
                  bottomLeft: Radius.circular(
                      MediaQuery.of(context).size.height * pullTabHeight)),
...
class ListViewPage extends StatefulWidget {
  ...
  ListViewPage(this.getCurrentLocationAndFindClosest, this.getCurrentLocation,
      this.reloadPage);
                    ...
                    child: Row(
                      children: <Widget>[
                        ...
                        currentStop != null
                            ? Container(
                                height: MediaQuery.of(context).size.width *
                                    (listViewTitleBarHeight - pullTabHeight) *
                                    0.6,
                                width: MediaQuery.of(context).size.width *
                                    (listViewTitleBarHeight - pullTabHeight) *
                                    0.6,
