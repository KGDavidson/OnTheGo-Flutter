...
Future<Stop> fetchFavouriteStop(String naptanId) async {
  ...
  if (response.statusCode == 200) {
    ...
    if (children[0]["children"].toList() != null) {
      List firstCheck = children.where((item) => item["naptanId"] == naptanId).toList().map((item) => Stop.fromJson(item)).toList();
      if (firstCheck.isNotEmpty) {
        return firstCheck[0];
      } else {
        return children
            .map((item) => item["children"].toList().where((item) => item["naptanId"] == naptanId).toList().map((item) => Stop.fromJson(item)).toList())
            .toList()
            .where((item) => item.length > 0)
            .toList()[0][0];
      }
    } else {
      return children.where((item) => item["naptanId"] == naptanId).toList().map((item) => Stop.fromJson(item)).toList()[0];
    }
  } else {
                    ...
                    children: <Widget>[
                        ...
                        children: [
                          Row(
                            children: <Widget>[
                              Container(
                                padding:
                                    EdgeInsets.only(left: MediaQuery.of(context).size.height * listViewTitleBarTextSize / 3, right: MediaQuery.of(context).size.height * listViewTitleBarTextSize / 3),
                                child: Text(
                                  currentStop != null
                                      ? currentStop.commonName.length > 17
                                          ? currentStop.commonName.replaceRange(18, currentStop.commonName.length, "...")
                              ...
                              currentStop != null
                                  ? GestureDetector(
                                      ...
                                      child: Container(
                                        ...
                                        child: Icon(
                                          currentFavourites.contains(currentStop.naptanId) ? Icons.favorite : Icons.favorite_border,
                            ...
                            child: currentStop != null
                                ? Container(
                                    padding: EdgeInsets.only(
                                        left: MediaQuery.of(context).size.height * listViewTitleBarTextSize / 3, top: MediaQuery.of(context).size.height * listViewTitleBarTextSize / 4),
                                    child: Text(
                      ...
                      currentStop != null
                          ? Container(
                              ...
                              child: Center(
                                  child: currentStop.stopLetter == null || currentStop.stopLetter.toString().contains("->") || currentStop.stopLetter == "Stop"
                                      ? Icon(
                                          Icons.train,
                                          color: Colors.white,
                                        )
                                      : Text(
                                          currentStop.stopLetter.split("Stop ")[1],
                                          style: TextStyle(
                                            color: Colors.white,
                                            fontWeight: FontWeight.bold,
                                            fontSize: MediaQuery.of(context).size.width * (listViewTitleBarHeight) * 0.6 * 0.4,
                                          ),
                                        )),
                            )
                          : Container(),
                          ...
                          child: SingleChildScrollView(
                            ...
                            child: Column(
                              children: currentArrivalTimes != null
                                  ? () {
                                      List returnArrivalTimes = currentArrivalTimes
